...
## 移除补全库 <Badge type="warning" text="v1.4前" />
`v1.4` 起 QuModLibs 已移除内置补全库，并完全改为使用系统补全库。以下工具主要用于处理旧版完整包体。
...
## 开发建议
`v1.4` 起已废弃 `MINI` 精简版，部分不常用模块移动到 `Optional/Modules`。当前建议是从标准包体出发，按需加入可选模块，并通过工具裁剪未使用模块。
