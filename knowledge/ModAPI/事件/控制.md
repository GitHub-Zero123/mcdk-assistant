...
| HoldBeforeClientEvent | 客户端 | 玩家长按屏幕，即将响应到游戏内时触发。仅在移动端或pc的F11模式下触发。pc的非F11模式可以监听[RightClickBeforeClientEvent](#rightclickbeforeclientevent)事件 |
...
| TapBeforeClientEvent | 客户端 | 玩家点击屏幕并松手，即将响应到游戏内时触发。仅在移动端或pc的F11模式下触发。pc的非F11模式可以监听[LeftClickBeforeClientEvent](#leftclickbeforeclientevent)事件 |
| TapOrHoldReleaseClientEvent | 客户端 | 玩家点击屏幕后松手时触发。仅在移动端或pc的F11模式下触发。pc的非F11模式可以使用[LeftClickReleaseClientEvent](#leftclickreleaseclientevent)与[RightClickReleaseClientEvent](#rightclickreleaseclientevent)事件监听鼠标松开 |
...
- 备注
  ...
  - 玩家长按屏幕的处理顺序为：
    ...
    1. 触发该事件
    1. 若事件没有cancel，则根据主手上的物品，准心处的物体类型以及与玩家的距离，进行挖方块/使用物品/与实体交互等操作
...
- 备注
  ...
  - 玩家点击屏幕的处理顺序为：
    ...
    1. 触发该事件
    1. 若事件没有cancel，则根据准心处的物体类型以及与玩家的距离，进行攻击或放置等操作
  - 与GetEntityByCoordEvent事件不同的是，被ui层捕获，没有穿透到世界的点击不会触发该事件，例如：
    1. 点击原版的移动/跳跃等按钮
    1. 通过SetIsHud(0)屏蔽了游戏操作
    1. 对按钮使用AddTouchEventHandler接口时isSwallow参数设置为True
  - 点触流程的图片讲解见[TapOrHoldReleaseClientEvent](#taporholdreleaseclientevent)事件的备注
...
- 备注
  ...
  - 短按及长按后松手都会触发该事件
  - 一张图读懂点触和按键事件触发流程
    ![点触说明](https://mc.163.com/dev/mcmanual/mc-dev/assets/img/pe_touch_event.d0013c68.png)
