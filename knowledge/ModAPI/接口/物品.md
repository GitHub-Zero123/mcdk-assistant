...
| 参数名 | 数据类型 | 说明 |
| --- | --- | --- |
| itemDict | dict | 物品信息字典，格式{'itemName': str, 'auxValue': int} |

- 返回值

| 数据类型 | 说明 |
| --- | --- |
| `tuple(float,float,float,float)` | 物品图标在图集中的UV坐标与尺寸(u0,v0,w,h)，u0,v0为左上角归一化UV坐标，w为纹理宽度，h为纹理高度。获取失败返回None |

- 备注

  - 获取的是物品图标所在图集的UV信息，方块类物品的图标位于atlas.terrain图集，普通物品图标位于atlas.items图集，两者返回的均为归一化UV坐标
...
import mod.client.extraClientApi as clientApi
comp = clientApi.GetEngineCompFactory().CreateItem(levelId)
uv = comp.GetIconInfo({"itemName": "minecraft:apple", "auxValue": 0})
# uv = (0.1064453125, 0.0009765625, 0.015625, 0.015625)
...
| 参数名 | 数据类型 | 说明 |
| --- | --- | --- |
| itemTag | str | 物品tag名，如 common:apple |

- 返回值

| 数据类型 | 说明 |
| --- | --- |
| `list(dict)` | 物品字典列表，dict包含itemName键 |
...
import mod.server.extraServerApi as serverApi
comp = serverApi.GetEngineCompFactory().CreateItem(levelId)
comp.LookupItemsByItemTag("common:apple")
