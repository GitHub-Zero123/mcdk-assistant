...
- 备注
  ...
  - 自定义方块实体外观的render/姿态（offset、rotation、scale、multipass）依赖客户端运行时才能获取。因此：
    1. 外观尚不支持动画，渲染效果为不带动画的默认姿态；
    1. 若调色板数据由服务端接口序列化产出，则该类方块的自定义外观会退化为默认姿态。
    1. 建议使用客户端接口序列化dataDict（GetBlockPaletteBetweenPos/GetBlockPaletteFromPosList 客户端版）。
  - 目前尚不支持的方块类型有 (后续版本将陆续支持)：
    1. 微缩方块（见开发指南-玩法开发-自定义游戏内容-微缩方块）
    1. 屏障
    1. 末地传送门，末地折跃门
...
| 参数名 | 数据类型 | 说明 |
| --- | --- | --- |
| blockPalette | BlockPaletteComponent | 方块调色板，由GetBlockPaletteBetweenPos、GetBlockPaletteFromPosList、以及GetBlankBlockPalette+DeserializeBlockPalette接口获取 |
| geometryName | str | 几何体模型的名称，用于标识每个几何体模型，相当于是该模型的id。 |
| unsupportedMode | int | 不支持方块处理方式枚举，可选参数，可输入的值为0、1。默认的值为0。如果方块调色板中有不支持转变为几何体的方块的话，将按以下方式处理：0：跳过不支持的方块继续生成。1：停止生成 |
...
## SetActorBlockGeometryRotationQuat
...
| 参数名 | 数据类型 | 说明 |
| --- | --- | --- |
| geometryName | str | 几何体模型的名称 |
| quat | `tuple(float,float,float,float)` | 四元数(x,y,z,w)，默认为(0,0,0,1)即无旋转。 |
