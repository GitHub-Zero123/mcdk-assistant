...
| face | int | 需要获取的方块面，参考Facing枚举，face默认值为6，此时获取方块所有面的贴图信息 |
...
| 参数名 | 数据类型 | 说明 |
| --- | --- | --- |
| textureKey | str | 纹理在terrain_texture.json中texture_data下的key值，如grass_top |

- 返回值

| 数据类型 | 说明 |
| --- | --- |
| `tuple(float,float,float,float)` | 纹理在图集中的UV坐标与尺寸(u0,v0,w,h)，u0,v0为左上角归一化UV坐标，w为纹理宽度，h为纹理高度。获取失败返回None |

- 备注

  - textureKey为方块贴图名name，方块所用的贴图名见blocks.json，textureKey值见资源包terrain_texture.json的texture_data
  - 该接口只支持atlas.terrain图集（方块图集），物品图标图集（atlas.items）不适用，获取物品图标UV请使用GetIconInfo
...
import mod.client.extraClientApi as clientApi
comp = clientApi.GetEngineCompFactory().CreateBlockInfo(levelId)
uv = comp.GetTextureAtlasUV("grass_top")
# uv = (0.80078125, 0.26953125, 0.0078125, 0.0078125)
