...
| GetModConfigJson | 服务端 | 以字典形式返回指定路径的json格式配置文件的内容，文件必须放置在行为包的/modconfigs目录下 |
...
| DestroyCrackFrame | 客户端 | 销毁特定方块位置上的破坏纹理（仅能销毁SetCrackFrame接口创建的破坏纹理）。 |
...
| SetCrackFrame | 客户端 | 仅客户端的破坏纹理的渲染，可自定义破坏阶段在第几帧。 |
...
| SetEntityScale | 服务端 | 设置实体的放缩比例大小，设置比例过大会导致游戏卡顿，建议控制在20倍以内。实体的scale的立方乘以包围盒的体积不可超过32768 |
...
| 接口 | | 描述 |
| --- | --- | --- |
| GetHostPlayerUid | 服务端 | 获取服务器拥有者的uid。 |
