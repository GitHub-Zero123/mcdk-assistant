...
| --- | --- | --- |
| modelName | str | 模型名称 |
| dimensionId | int | 维度id，-1表示全维度可见，0表示主世界，1表示地狱，2表示末地。默认-1 |
...
# 创建仅在主世界可见的自由模型
model = comp.CreateFreeModel(modelName, 0)
...
| 参数名 | 数据类型 | 说明 |
| --- | --- | --- |
| brightness | float | 0：纯黑 1：正常亮度 1-14：较亮甚至纯白 超过14：通常为纯白，即使数值改变也没有明显变化 |
...
| --- | --- | --- |
| modelId | int | 要设置的modelId |
| dimensionId | int | 维度id，-1表示全维度可见，0表示主世界，1表示地狱，2表示末地 |
...
comp = clientApi.GetEngineCompFactory().CreateModel(levelId)
# 将模型设置为仅在主世界可见
comp.SetFreeModelDimensionId(modelId, 0)
# 将模型设置为全维度可见
comp.SetFreeModelDimensionId(modelId, -1)
