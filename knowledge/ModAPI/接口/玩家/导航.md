...
| 数据类型 | 说明 |
| --- | --- |
| int或list(`tuple(float,float,float)`) | 返回1：参数错误 返回2：玩家所在chunk未加载完毕 返回3：终点为实心方块，无法寻路 返回list(`tuple(float,float,float)`,)：从起点到终点的坐标点列表。注意该list可能为空，表示本地玩家离地太远，或者被堵住无法行动。 |
...
| 数据类型 | 说明 |
| --- | --- |
| int | 返回0：导航正常开始 返回-1：本地玩家离地太远，或者被堵住无法行动 返回1：参数错误 返回2：玩家所在chunk未加载完毕 返回3：终点为实心方块，无法寻路 |
