...
NetEase Minecraft / Bedrock 开发场景通用 MCP Server。

为 Claude Desktop、Cursor、VS Code、CodeMaker 等支持 MCP 的 AI 客户端提供文档检索、原版资源搜索、JSON UI 分析、模型与动画编辑、像素画处理等能力，让 AI 可以直接参与 Minecraft 开发工作流，而不只是做纯文本回答。
...
JSON UI 工具 | 查询控件结构、搜索属性、诊断问题、补丁修改、生成 UI 模板
模型工具 | 解析和编辑 `geometry.json`，支持骨骼编辑、镜像建模、定位点操作
动画工具 | 解析和编辑 `animations.json`，支持关键帧修改、时间缩放、镜像动作
像素画工具 | 支持画布读写、批量绘制、裁切缩放、描边阴影、调色板提取、抖动量化
参考速查 | 快速获取网易版差异、JSON UI / 动画 / 模型参考资料

当前完整版本内置约 `55` 个 MCP 工具。

## 适用场景

- 查询网易版独占资料、原版资源、组件和接口参考
- 让 AI 辅助分析和修改 JSON UI
- 让 AI 辅助分析和编辑模型、动画资源
- 生成和修改像素风贴图
- 快速说明网易版与国际版差异
...
当前 VSCode 不能直接连接 SSE，可通过 `mcp-remote` 做桥接：
...
以下能力涉及本地文件读写、资源修改或画布操作，不包含在默认发布的 LITE 版本中：
...
动画 | `animation_parse`、`animation_get_bone_channel`、`anim_op`
