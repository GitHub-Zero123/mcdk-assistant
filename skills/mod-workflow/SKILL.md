---
name: mod-workflow
description: 网易基岩版 MOD Python 开发强制工作流。未知项目必须先 scan_py2_mod_architecture 建立认知；改动已有模块必须先 scan_py2_import_chain 分析引用链；禁止单文件堆积；禁止跨端侧 import；事件监听优先注册表分发避免重复注册；mcmodsdk 环境默认按 str 字节串处理，非必要不转 unicode；游戏业务非必要不 try 且不吞异常；关键决策点必须确认用户意图。
---
...
mcmodsdk / 网易 Py2 MOD 运行环境中，游戏 API 与业务数据大多以 `str` 字节串形式传递，而不是 `unicode`。

**硬性规则**：

- 非必要场景不得主动把普通业务字符串转为 `unicode`
- 不得为所有字符串处理添加通用的 `unicode` 兜底转换
- 只有在确实需要按字符语义处理时，才允许临时转换为 `unicode`

**允许临时转换的典型场景**：

- 需要明确获取中文字符数量，而不是字节长度
- 需要按字符进行切片、截断、对齐或逐字遍历
- 需要与明确要求 `unicode` 的第三方库或接口交互

**决策规则**：

- 如果只是传递 identifier、事件参数、配置 key、资源路径、命令参数、英文/命名空间 ID，保持 `str`，不要转换
- 如果只是比较、字典查找、拼接、格式化日志，优先保持 `str`
- 如果必须按中文字符数处理，转换范围必须尽量小：用完即回到项目原有字符串约定

**禁止**：

- 禁止把 `unicode(value, "utf-8")` 或等价转换作为所有字符串入口的默认兜底
- 禁止在没有字符级处理需求时，为了“保险”同时兼容 `str` / `unicode`
- 禁止引入大量编码兜底逻辑掩盖真实数据约定
...
收到任务
  ...
  │   ├─ 事件监听优先注册表分发 【2.5】
  │   ├─ 非必要不 try，不吞异常 【2.6】
  │   └─ 非必要不转 unicode，默认保持 str 字节串 【2.7】
  │
  ├─ 5. 【阶段三】改动后验证（用工具确认）
