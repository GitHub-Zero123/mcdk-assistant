...
review --dump-config    打印可编辑的默认配置模板（TOML，兼作阈值/开关说明文档）
    ...
    导入违反线上平台安全规则的模块、依赖 Minecraft 原版内部 API（不承诺稳定性，不推荐使用），
    动态导入或执行代码，以及通过反射和间接变量链取得敏感解释器能力。
    安全规则的具体匹配清单和绕过特征不在帮助文本中公开；finding 仅报告实际命中的代码位置。
    QuModLibs 与 ModSDK 公开 API 不误报（开放世界）。
